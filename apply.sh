...
original_author="NExPlain"
original_name="api_gpt"
original_urlname="API-GPT"
original_description="Awesome api_gpt created by NExPlain" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/api_gpt "${name}"
...
then
    ...
    echo "then run 'api_gpt' or 'python -m api_gpt'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'api_gpt' or 'python -m api_gpt' or 'poetry run api_gpt'"
fi
